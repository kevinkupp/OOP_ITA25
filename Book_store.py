...
class Store:
    ...
    def can_add_book(self, book: Book) -> bool:
        ...
        2. book's own rating is >= than store's rating
         ...
         :return: bool
        """
        for some_book in self.books:
            if some_book.title == book.title and some_book.author == book.author:
                return False

        if self.rating >= book.rating:
            return False
    ...
    def get_books_by_price(self) -> list:
        ...
        :return: list of Book objects
        """
        books_price = sorted(self.books, key=lambda book: book.price)
        return books_price
